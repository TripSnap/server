...
@Transactional
@DisplayName("GroupMemberRequest 엔티티 테스트")
class GroupMemberRequestTest {
    ...
    void key() {
        ...
        List<GroupMemberRequest> requests = ids.stream()
                .map(id -> GroupMemberRequest.builder().id(id).group(finalGroup).member(memberMap.get(id.getMemberId())).build())
                .collect(Collectors.toList());
        ReflectionTestUtils.setField(group,"owner",  memberMap.get(member1Id));
        ReflectionTestUtils.setField(group,"memberRequests",  requests);
        ...
        Assertions.assertFalse(group.getMemberRequests().isEmpty());
        for(GroupMemberRequest m : requests) {
            Assertions.assertTrue(group.getMemberRequests().stream().anyMatch(member -> member.getId().equals(m.getId())));
        }
